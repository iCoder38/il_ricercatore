//
//  health_reminder.swift
//  IL-Ricercatore
//
//  Created by Dishant Rajput on 22/07/24.
//

import UIKit

class health_reminder: UIViewController, UNUserNotificationCenterDelegate {
    ...
    @IBOutlet weak var lbl_navigation_title:UILabel! {
        didSet {
            lbl_navigation_title.text = "HEALTH REMINDER"
    ...
    override func viewDidLoad() {
        ...
        self.btn_reminde_me_every_at_checkmark.addTarget(self, action: #selector(remind_me_every_at_click_method), for: .touchUpInside)
        ...
        UNUserNotificationCenter.current().getPendingNotificationRequests { (requests) in
            ...
            for request in requests {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [self] in
                    if ("\(request.identifier)") == identifier_health_reminder_remind_me_every_at {
                    ...
                    if ("\(request.identifier)") == identifier_health_reminder_remind_me_only1 {
    ...
    @objc func btn_remind_me_only_d_t_click_method() {
        ...
        RPicker.selectDate(title: "Select date", cancelText: "Cancel", datePickerMode: .dateAndTime,minDate: Date.now, didSelectDate: { (selectedDate) in
            ...
            content.title = local_notification_health_reminder_header
            content.body = local_notification_health_reminder_body
            ...
            let request = UNNotificationRequest(identifier: identifier_health_reminder_remind_me_only1, content: content, trigger: trigger)
    ...
    @objc func btn_remind_me_checkmark_click_method() {
        ...
        if (self.btn_remind_me_day_checkmark.tag == 0) {
            ...
            center.removePendingNotificationRequests(withIdentifiers: [identifier_health_reminder_remind_me_every_at])
    ...
    @objc func time_picker() {
        ...
        RPicker.selectDate(title: "Select Time", cancelText: "Cancel", datePickerMode: .time, didSelectDate: { (selectedDate) in
            ...
            let merge = String(self.str_day_name)+" | "+String(selectedDate.dateString("hh:mm a"))
            self.btn_remind_me_day_time.setTitle(String(merge), for: .normal)
            
            self.weekday_reminder(header: local_notification_health_reminder_header,
                                  body: local_notification_health_reminder_body,
                                  day: self.str_day_count,
                                  get_full_time: String(selectedDate.dateString("HH:mm")),
                                  identifier: identifier_health_reminder_remind_me_every_at,
    ...
    @objc func disbale_remind_me_every_at() {
        ...
        self.btn_reminde_me_every_at_time.tag = 0
        self.btn_reminde_me_every_at_checkmark.setImage(UIImage(named: "uncheck"), for: .normal)
        self.btn_reminde_me_every_at_time.backgroundColor = .systemGray5
        self.btn_reminde_me_every_at_time.isUserInteractionEnabled = false
        self.btn_reminde_me_every_at_time.setTitle("disable", for: .normal)
        self.str_remind_me_every_at_status = "0"
        
        // disable reminder remind me at
        let center = UNUserNotificationCenter.current()
        center.removePendingNotificationRequests(withIdentifiers: [identifier_walk_reminder_remind_me_every_at])
    }
    
    @objc func remind_me_every_at_click_method() {
        
        if (self.btn_reminde_me_every_at_time.tag == 0) {
            self.btn_reminde_me_every_at_time.tag = 1
            self.btn_reminde_me_every_at_checkmark.setImage(UIImage(named: "check"), for: .normal)
            self.btn_reminde_me_every_at_time.backgroundColor = .systemPurple
            self.btn_reminde_me_every_at_time.isUserInteractionEnabled = true
            self.btn_reminde_me_every_at_time.setTitle("select", for: .normal)
            self.str_remind_me_every_at_status = "1"
            // action
            self.btn_reminde_me_every_at_time.addTarget(self, action: #selector(time_picker_remind_me_every_at), for: .touchUpInside)
            ...
            // disable reminder
            self.disable_reminder(identifier: identifier_health_reminder_remind_me_every_at, text: type_workout_remind_me_every_at)
        }
    ...
    @objc func time_picker_remind_me_every_at() {
        ...
        RPicker.selectDate(title: "Select Time", cancelText: "Cancel", datePickerMode: .time, didSelectDate: { (selectedDate) in
            self.btn_reminde_me_every_at_time.setTitle(selectedDate.dateString("hh:mm a"), for: .normal)
            self.str_selected_time = selectedDate.dateString("HH:mm")
            
            
            
            self.open_custom_day(header: local_notification_walk_reminder_header,
                                 body: local_notification_walk_reminder_body,
                                 identifier: identifier_walk_reminder_remind_me_every_at,
